...
async def text_to_speech(request: dict):
    """
    Receives text and returns an MP3 audio file generated by Fish Audio.
    """
    text = request.get("text", "").strip()
    if not text:
        raise HTTPException(400, detail="📝 No text provided for TTS.")
    
    if not Config.FISH_API_KEY:
        raise HTTPException(503, detail="Fish Audio API key is not configured on the server.")

    try:
        # The FishAudio client will automatically read the FISH_API_KEY from the environment
        client = FishAudio()
        
        # Generate the audio using the free model
        audio_data = client.tts.convert(
            text=text,
            model="s2.1-pro-free",
            format="mp3" # We'll request MP3 for broad browser support
        )
        ...
        raise HTTPException(500, detail="🎤 Text-to-speech generation failed.")
